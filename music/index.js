...
const db_connection = "mongodb://localhost:27017/music"
const port = 2324
...
// app.post("/add/:Songname/:Film/:Music_director/:singer/:Actor/:Actress", async function (request, response) {
//     // get data from body
...
// GET request to add a new song
app.get("/add/:Songname/:Film/:Music_director/:singer/:Actor/:Actress", async function (request, response) {
    const { Songname, Film, Music_director, singer, Actor, Actress } = request.params;
...
app.get("/getSongsOfDirector/:directorName", async function (request, response) {
    ...
    response.send("Songs by given music director are :" , songs)
})
